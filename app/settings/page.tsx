    ...
    <div className="px-4 pt-6 pb-4 space-y-6">
      ...
      <Section title="Master Resume" description="Your base resume. All tailored drafts are generated from this.">
        <textarea
          ...
          rows={14}
          className="w-full bg-white/5 border border-white/10 focus:border-violet-500 rounded-lg px-3 py-2.5 text-sm text-white placeholder-slate-600 focus:outline-none resize-none font-mono"
        />
      ...
      >
        <textarea
          ...
          rows={6}
          className="w-full bg-white/5 border border-white/10 focus:border-violet-500 rounded-lg px-3 py-2.5 text-sm text-white placeholder-slate-600 focus:outline-none resize-none"
        />
      </Section>
      ...
      >
        {/* File upload */}
        <label className={`flex items-center justify-center gap-2 w-full border border-dashed rounded-lg py-3 text-sm transition-colors cursor-pointer ${parsing ? 'border-white/10 text-slate-500' : 'border-white/10 hover:border-violet-500/60 text-slate-400 hover:text-violet-400'}`}>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-slate-500 text-xs">or paste text</span>
          <div className="flex-1 border-t border-white/5" />
        </div>
        ...
        <button
          ...
          className="w-full bg-white/10 hover:bg-white/15 border border-white/20 disabled:opacity-40 text-white text-sm font-medium py-2.5 rounded-lg transition-colors"
        >
          {extracting ? 'Extracting new facts...' : 'Extract & Add to Fact Bank'}
        </button>
        {extractResult && (
          <div className={`rounded-lg px-3 py-2.5 text-xs ${extractResult.startsWith('No new') ? 'bg-white/5 text-slate-400' : 'bg-emerald-500/10 border border-emerald-500/20 text-emerald-300'}`}>
        ...
        {extractError && (
          <p className="text-rose-400 text-xs">{extractError}</p>
        )}
      </Section>
      ...
      <Section title="Generation Settings" description="Controls for automated pipeline behavior.">
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            />
            <p className="text-slate-500 text-[10px] mt-1">Max resumes per day</p>
          </div>
          <div>
            ...
            <input
              ...
              className="w-full bg-white/5 border border-white/10 focus:border-violet-500 rounded-lg px-3 py-2 text-sm text-white focus:outline-none"
            />
            <p className="text-slate-500 text-[10px] mt-1">Min AI score (0–100)</p>
          </div>
        </div>
      </Section>
      ...
      {saveError && (
        <p className="text-rose-400 text-xs rounded-lg bg-rose-600/20 border border-rose-500/40 px-3 py-2">
          {saveError}
        </p>
      )}
      ...
      <button
        onClick={handleSave}
        disabled={saving}
        className="w-full bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 disabled:opacity-50 text-white text-sm font-medium py-3 rounded-lg transition-all shadow-lg shadow-violet-900/40"
      >
      ...
      <div className="border-t border-white/5 pt-4">
        <button
          ...
          className="w-full text-slate-500 hover:text-rose-400 text-sm py-2 transition-colors"
        >
          Sign out
        </button>
      </div>
